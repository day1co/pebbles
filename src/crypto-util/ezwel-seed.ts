...
export class EzwelCrypto {
  ...
  public static SeedEncrypt(
    ...
    // Because JavaScript engines use big endian order in default
    if (!this.ENDIAN) {
  ...
  private static EncRoundKeyUpdate0(
    ...
    let T0: number, T00: number, T11: number;
    T0 = A[0];
    A[0] = (A[0] >>> 8) ^ (B[0] << 24);
    B[0] = (B[0] >>> 8) ^ (T0 << 24);
    T00 = A[0] + C[0] - KC[Z];
    T11 = B[0] + KC[Z] - D[0];
    K[0] = SS0[this.GetB0(T00)] ^ SS1[this.GetB1(T00)] ^ SS2[this.GetB2(T00)] ^ SS3[this.GetB3(T00)];
    K[1] = SS0[this.GetB0(T11)] ^ SS1[this.GetB1(T11)] ^ SS2[this.GetB2(T11)] ^ SS3[this.GetB3(T11)];
  }
  ...
  /******************************** Key Schedule ********************************/
  public static SeedRoundKey(
    pdwRoundKey: number[],
    pbUserKey: Uint8Array
  ): void {
    const A: number[] = [0];
    const B: number[] = [0];
    const C: number[] = [0];
    ...
    pdwRoundKey[0] = SS0[this.GetB0(T0)] ^ SS1[this.GetB1(T0)] ^ SS2[this.GetB2(T0)] ^ SS3[this.GetB3(T0)];
    pdwRoundKey[1] = SS0[this.GetB0(T1)] ^ SS1[this.GetB1(T1)] ^ SS2[this.GetB2(T1)] ^ SS3[this.GetB3(T1)];
   ...
   */
  public encrypt(sbuffer: Uint8Array | string, szKey: Uint8Array): Uint8Array {
    ...
    for (let j = 0; j < rt; j++) {
      ...
      const sTarget = new Uint8Array(this.SeedBlockSize);

      sSource.set(inDataBuffer.slice(j * this.SeedBlockSize, (j + 1) * this.SeedBlockSize));
      EzwelCrypto.SeedEncrypt(sSource, sRoundKey, sTarget);
   ...
   *
   * @param inData - string to be encrypted
   * @param szKey - key for encryption
   * @returns encrypted data
   */
  public encryptString(inData: string, szKey: Uint8Array): Uint8Array {
    return this.encrypt(inData, szKey);
  }

  /**
   * Encrypts the provided string using the key and specified character set
   *
   * @param inData - string to be encrypted
   * @param szKey - key for encryption
   * @param charset - charset to use for string conversion
   * @returns encrypted data
   */
  public encryptStringWithCharset(
    inData: string,
    szKey: Uint8Array,
    charset: string
  ): Uint8Array {
    const encoder = new TextEncoder(); // TextEncoder uses UTF-8 by default
    // Note: In JavaScript, specifying different character encodings is more complex
    // This implementation defaults to UTF-8, which may not match the Java version's behavior
    // A more complete implementation would need a custom encoding solution for other charsets
    const bytes = encoder.encode(inData);
    return this.encrypt(bytes, szKey);
  }
   ...
   */
  public decrypt(encryptBytes: Uint8Array, szKey: Uint8Array): Uint8Array {
    const sRoundKey: number[] = new Array(32);
    ...
    for (let j = 0; j < rt; j++) {
      ...
      EzwelCrypto.SeedDecrypt(sSource, sRoundKey, sTarget);
