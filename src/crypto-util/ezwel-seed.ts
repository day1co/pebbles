...
export class EzwelCrypto {
  ...
  /***************************** SEED round function ****************************/
  private static SeedRound(L0: number[], L1: number[], R0: number[], R1: number[], K: number[]): void {
  ...
  private static EncRoundKeyUpdate0(K: number[], A: number[], B: number[], C: number[], D: number[], Z: number): void {
    const T0 = A[0];
    A[0] = (A[0] >>> 8) ^ (B[0] << 24);
    B[0] = (B[0] >>> 8) ^ (T0 << 24);
  ...
  private static EncRoundKeyUpdate1(K: number[], A: number[], B: number[], C: number[], D: number[], Z: number): void {
    const T0 = C[0];
    C[0] = (C[0] << 8) ^ (D[0] >>> 24);
    D[0] = (D[0] << 8) ^ (T0 >>> 24);
    const T00 = A[0] + C[0] - KC[Z];
