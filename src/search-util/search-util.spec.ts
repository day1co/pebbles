import { binarySearch } from './search-util';
...
describe('binarySearch', () => {
  describe('number array', () => {
    ...
    it('should find an existing number', () => {
      const result = binarySearch(numbers, 7, compareFn);
      expect(result).toBe(7);
    });

    it('should return undefined for a non-existing number', () => {
      const result = binarySearch(numbers, 6, compareFn);
      expect(result).toBeUndefined();
    });

    it('should find the first element', () => {
      const result = binarySearch(numbers, 1, compareFn);
      expect(result).toBe(1);
    });

    it('should find the last element', () => {
      const result = binarySearch(numbers, 15, compareFn);
  ...
  describe('string array', () => {
    ...
    it('should find an existing fruit', () => {
      const result = binarySearch(fruits, 'cherry', compareFn);
      expect(result).toBe('cherry');
    });

    it('should return undefined for a non-existing fruit', () => {
      const result = binarySearch(fruits, 'grape', compareFn);
      expect(result).toBeUndefined();
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle an empty array', () => {
      const result = binarySearch([], 1, (target, item) => target - item);
      expect(result).toBeUndefined();
    });

    it('should handle an array with one element (found)', () => {
      const result = binarySearch([1], 1, (target, item) => target - item);
      expect(result).toBe(1);
    });

    it('should handle an array with one element (not found)', () => {
      const result = binarySearch([1], 2, (target, item) => target - item);
      expect(result).toBeUndefined();
    });
  });
});
